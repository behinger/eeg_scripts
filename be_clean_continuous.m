...
if exist(p.reject(sub).continuous,'file')==2
    ...
    if exist(p.reject(sub).continuous,'file')==2 && ~silent && ~strcmp(cleaningInput,'u') || resave % if we find the file, and we are not on the grid and we do not continue without cleaning anything, backup!
